...
var X_FORWARDED_HEADERS = /* @__PURE__ */ new Set([
  "x-forwarded-for",
  "x-forwarded-host",
  "x-forwarded-port",
  "x-forwarded-proto",
...
function requireStringifier() {
  ...
  class Stringifier {
    ...
    block(node2, start) {
      let raws = node2.raws;
      let between = typeof raws.between !== "undefined" ? raws.between : this.raw(node2, "between", "beforeOpen");
      this.builder(escapeHTMLInCSS(start + between) + "{", node2, "start");
      let after;
      if (node2.nodes && node2.nodes.length) {
        this.body(node2);
        after = typeof raws.after !== "undefined" ? raws.after : this.raw(node2, "after");
      } else {
        after = typeof raws.after !== "undefined" ? raws.after : this.raw(node2, "after", "emptyBody");
      }
    ...
    body(node2) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (before) this.builder(isDocument2 ? before : escapeHTMLInCSS(before));
        this.stringify(child, last !== i || semicolon);
      }
    }
    comment(node2) {
      let raws = node2.raws;
      let left = typeof raws.left !== "undefined" ? raws.left : this.raw(node2, "left", "commentLeft");
      let right = typeof raws.right !== "undefined" ? raws.right : this.raw(node2, "right", "commentRight");
      this.builder(escapeHTMLInCSS("/*" + left + node2.text + right + "*/"), node2);
    }
    decl(node2, semicolon) {
      let raws = node2.raws;
      let between = typeof raws.between !== "undefined" ? raws.between : this.raw(node2, "between", "colon");
      let rawVal = raws.value;
      let value = rawVal && rawVal.value === node2.value ? rawVal.raw : node2.value;
      let string = node2.prop + between + value;
      if (node2.important) {
...
var VALID_REDIRECT_RESPONSE_CODES$1 = /* @__PURE__ */ new Set([301, 302, 303, 307, 308]);
    ...
    try {
      ...
      if (metadata.renderMode === RenderMode.Prerender) {
        ...
        if (metadata.status && !VALID_REDIRECT_RESPONSE_CODES$1.has(metadata.status)) {
          yield {
            error: `The '${metadata.status}' status code is not a valid redirect response code. Please use one of the following redirect response codes: ${[...VALID_REDIRECT_RESPONSE_CODES$1.values()].join(", ")}.`
          };
...
//# sourceMappingURL=chunk-EATN6WP5.js.map
