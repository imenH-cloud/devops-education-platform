...
import {
  BidiModule,
  Directionality
} from "./chunk-FZ7X4IZA.js";
import {
  Location,
  isPlatformBrowser
} from "./chunk-EDQPIAAB.js";
...
var BreakpointObserver = class _BreakpointObserver {
  ...
  _destroySubject = new import_rxjs2.Subject();
  constructor() {
  }
...
var ContentObserver = class _ContentObserver {
  ...
  observe(elementOrRef) {
    ...
    return new import_rxjs3.Observable((observer) => {
      ...
      const subscription = stream.pipe((0, import_operators3.map)((records) => records.filter((record) => !shouldIgnoreRecord(record))), (0, import_operators3.filter)((records) => !!records.length)).subscribe((records) => {
        this._ngZone.run(() => {
...
var CdkObserveContent = class _CdkObserveContent {
  ...
  _subscribe() {
    ...
    this._currentSubscription = (this.debounce ? stream.pipe((0, import_operators3.debounceTime)(this.debounce)) : stream).subscribe(this.event);
  }
  _unsubscribe() {
...
var FixedSizeVirtualScrollStrategy = class {
  _scrolledIndexChange = new import_rxjs6.Subject();
  /** @docs-private Implemented as part of VirtualScrollStrategy. */
  scrolledIndexChange = this._scrolledIndexChange.pipe((0, import_operators4.distinctUntilChanged)());
...
var ScrollDispatcher = class _ScrollDispatcher {
  ...
  _scrolled = new import_rxjs6.Subject();
  ...
  ancestorScrolled(elementOrElementRef, auditTimeInMs) {
    ...
    return this.scrolled(auditTimeInMs).pipe((0, import_operators4.filter)((target) => !target || ancestors.indexOf(target) > -1));
  }
...
var CdkScrollable = class _CdkScrollable {
  ...
  _destroyed = new import_rxjs6.Subject();
  _renderer = inject(Renderer2);
  _cleanupScroll;
  _elementScrolled = new import_rxjs6.Subject();
  constructor() {
  }
  ngOnInit() {
...
var ViewportRuler = class _ViewportRuler {
  ...
  _change = new import_rxjs6.Subject();
  /** Used to reference correct document/window */
  _document = inject(DOCUMENT);
  constructor() {
    const ngZone = inject(NgZone);
...
var CdkVirtualScrollViewport = class _CdkVirtualScrollViewport extends CdkVirtualScrollable {
  ...
  _detachedSubject = new import_rxjs6.Subject();
  /** Emits when the rendered range changes. */
  _renderedRangeSubject = new import_rxjs6.Subject();
  ...
  scrolledIndexChange = new import_rxjs6.Observable((observer) => this._scrollStrategy.scrolledIndexChange.subscribe((index) => Promise.resolve().then(() => this.ngZone.run(() => observer.next(index)))));
  ...
  _viewportChanges = import_rxjs6.Subscription.EMPTY;
  _injector = inject(Injector);
  _isDestroyed = false;
  constructor() {
    super();
...
var CdkVirtualForOf = class _CdkVirtualForOf {
  ...
  viewChange = new import_rxjs6.Subject();
  /** Subject that emits when a new DataSource instance is given. */
  _dataSourceChanges = new import_rxjs6.Subject();
...
var CdkDialogContainer = class _CdkDialogContainer extends BasePortalOutlet {
  ...
  _portalOutlet;
  _focusTrapped = new import_rxjs8.Subject();
...
var DialogRef = class {
  ...
  closed = new import_rxjs8.Subject();
...
var Dialog = class _Dialog {
  ...
  _afterAllClosedAtThisLevel = new import_rxjs8.Subject();
  _afterOpenedAtThisLevel = new import_rxjs8.Subject();
   ...
   */
  afterAllClosed = (0, import_rxjs8.defer)(() => this.openDialogs.length ? this._getAfterAllClosed() : this._getAfterAllClosed().pipe((0, import_operators6.startWith)(void 0)));
  constructor() {
  }
  open(componentOrTemplateRef, config) {
    ...
    if (!this.openDialogs.length) {
      ...
      if (dialogContainer._focusTrapped) {
        dialogContainer._focusTrapped.pipe((0, import_operators6.take)(1)).subscribe(() => {
...
var Typeahead = class {
  _letterKeyStream = new import_rxjs11.Subject();
  _items = [];
  _selectedItemIndex = -1;
  /** Buffer for the letters that the user has pressed */
  _pressedLetters = [];
  _skipPredicateFn;
  _selectedItem = new import_rxjs11.Subject();
  ...
  _setupKeyHandler(typeAheadInterval) {
    this._letterKeyStream.pipe((0, import_operators9.tap)((letter) => this._pressedLetters.push(letter)), (0, import_operators9.debounceTime)(typeAheadInterval), (0, import_operators9.filter)(() => this._pressedLetters.length > 0), (0, import_operators9.map)(() => this._pressedLetters.join("").toLocaleUpperCase())).subscribe((inputString) => {
...
function coerceObservable(data) {
  if (!(0, import_rxjs13.isObservable)(data)) {
    return (0, import_rxjs13.of)(data);
  }
  return data;
}
...
var TreeKeyManager = class {
  ...
  _items = [];
  _typeahead;
  _typeaheadSubscription = import_rxjs14.Subscription.EMPTY;
  ...
  constructor(items, config) {
    ...
    } else if ((0, import_rxjs14.isObservable)(items)) {
  ...
  change = new import_rxjs14.Subject();
  ...
  _expandCurrentItem() {
    ...
    if (!this._isCurrentItemExpanded()) {
      ...
      coerceObservable(this._activeItem.getChildren()).pipe((0, import_operators10.take)(1)).subscribe((children) => {
  ...
  _expandAllItemsAtCurrentItemLevel() {
    ...
    let itemsToExpand;
    if (!parent) {
      itemsToExpand = (0, import_rxjs14.of)(this._items.filter((item) => item.getParent() === null));
    } else {
      itemsToExpand = coerceObservable(parent.getChildren());
    }
    itemsToExpand.pipe((0, import_operators10.take)(1)).subscribe((items) => {
...
var MatDialogRef = class {
  ...
  _afterOpened = new import_rxjs16.Subject();
  /** Subject for notifying the user that the dialog has started closing. */
  _beforeClosed = new import_rxjs16.Subject();
  ...
  constructor(_ref, _config, _containerInstance) {
    ...
    _containerInstance._animationStateChanged.pipe((0, import_operators12.filter)((event) => event.state === "opened"), (0, import_operators12.take)(1)).subscribe(() => {
      this._afterOpened.next();
      this._afterOpened.complete();
    });
    _containerInstance._animationStateChanged.pipe((0, import_operators12.filter)((event) => event.state === "closed"), (0, import_operators12.take)(1)).subscribe(() => {
    ...
    (0, import_rxjs16.merge)(this.backdropClick(), this.keydownEvents().pipe((0, import_operators12.filter)((event) => event.keyCode === ESCAPE && !this.disableClose && !hasModifierKey(event)))).subscribe((event) => {
  ...
  close(dialogResult) {
    ...
    this._containerInstance._animationStateChanged.pipe((0, import_operators12.filter)((event) => event.state === "closing"), (0, import_operators12.take)(1)).subscribe((event) => {
...
var MatDialog = class _MatDialog {
  ...
  _openDialogsAtThisLevel = [];
  _afterAllClosedAtThisLevel = new import_rxjs16.Subject();
  _afterOpenedAtThisLevel = new import_rxjs16.Subject();
   ...
   * Will emit on subscribe if there are no open dialogs to begin with.
   */
  afterAllClosed = (0, import_rxjs16.defer)(() => this.openDialogs.length ? this._getAfterAllClosed() : this._getAfterAllClosed().pipe((0, import_operators12.startWith)(void 0)));
  constructor() {
...
var import_operators13 = __toESM(require_operators(), 1);
var _defaultParams = {
