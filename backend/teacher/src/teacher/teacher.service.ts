...
export class TeacherService {
  ...
  async create(createTeacherDto: CreateTeacherDto): Promise<Teacher> {
    try {
      console.log('📝 Received DTO:', JSON.stringify(createTeacherDto));
      
      // Ajouter des valeurs par défaut pour les champs vides
      const teacherData = {
        ...createTeacherDto,
        profileImage: createTeacherDto.profileImage || 'https://via.placeholder.com/150',
        facebook: createTeacherDto.facebook || '',
        instagram: createTeacherDto.instagram || '',
        linkedin: createTeacherDto.linkedin || '',
      };
      
      console.log('🔧 Processing data:', JSON.stringify(teacherData));
      const teacher = this.teacherRepository.create(teacherData);
      console.log('✅ Entity created:', teacher);
